 ...
 let accessToken = session?.accessToken;
  ...
  useEffect(() => {
    const fetchCateringData = async () => {
      setLoading(true);
      let loc_id = selectedLocationId || cateringData?.location_details?.id;
      try {
        if (loc_id !== undefined && loc_id !== null) {
          ...
          const sug = await api.get(`/catering/?location=${loc_id}`, config);
    ...
    fetchCateringData();
  }, [accessToken, cateringData, selectedLocationId]);
  ...
  useEffect(() => {
    ...
    fetChcateringData();
  }, [cateringId, accessToken]);

  useEffect(() => {
    if (cateringData?.name) {
      setContent(
        `${cateringData.name} is a delightful store offering a wide array of thoughtful presents for all your special occasions. The diverse selection of items available is mentioned below:`
      );
      setProductContent(
        `${cateringData?.name} is perfect for your guests or event needs.`
      );
      setContactInfo({
        name: cateringData?.manager_name || "N/A",
        number: cateringData?.contact_number || "N/A",
        alternate_number: cateringData?.alternative_number || "N/A",
      });
    }
  }, [cateringData]);
  ...
  return (
    <>
                    ...
                    <div className="pt-14 pb-10">
                      <div className="mt-10">
                        <ServicesOffered
                          ...
                          content={content}
                          data={cateringData?.services_offered_details}
                        />
      ...
      <SocialMediaLinks data={cateringData} />
    </>
  );
}
