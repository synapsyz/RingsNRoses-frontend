...
export default function Home() {
  ...
  const [entertainmentId, setEntertainmentId] = useState(null);
  ...
  useEffect(() => {
    if (router.isReady && router.query.id) {
      ...
      setEntertainmentId(router.query.id);
    }
  ...
  useEffect(() => {
    const fetchEntertainmentData = async () => {
      setLoading(true);
      let loc_id = selectedLocationId || entertainmentData?.location_details?.id;
      try {
        if (loc_id !== undefined && loc_id !== null) {
          ...
          const sug = await api.get(`/entertainment/?location=${loc_id}`, config);
  ...
  }, [accessToken, entertainmentData, selectedLocationId]);
  ...
  useEffect(() => {
    ...
    const fetchEntertainmentData = async () => {
      ...
      try {
        ...
        const response = await api.get(`/entertainment/${entertainmentId}/`, config);
    ...
    fetchEntertainmentData();
  }, [entertainmentId, accessToken]);

  useEffect(() => {
    if (entertainmentData?.name) {
      setContent(
        `${entertainmentData.name} is a delightful store offering a wide array of thoughtful presents for all your special occasions. The diverse selection of items available is mentioned below:`
      );
      setProductContent(
        `${entertainmentData?.name} is perfect for your guests or event needs.`
      );
      setContactInfo({
        name: entertainmentData?.manager_name || "N/A",
        number: entertainmentData?.contact_number || "N/A",
        alternate_number: entertainmentData?.alternative_number || "N/A",
      });
    }
  }, [entertainmentData]);
  ...
  return (
    <>
      <Head>
        <title>
          {entertainmentData?.name
            ? `${entertainmentData.name} | Entertainment & Favors`
            : "Loading Entertainment Details..."}
        </title>
        ...
        <main>
          ...
          <div id="content">
            <div className="w-full max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto">
              <div className="pt-6">
                <Breadcrumb data={entertainmentData} />
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-y-10">
                  <div className="lg:col-span-3">
                    <ImageSlider data={entertainmentData} />
                    ...
                    <div className="pt-14 pb-10">
                      <div className="mt-10">
                        <ServicesOffered
                          ...
                          content={content}
                          data={entertainmentData?.services_offered_details}
                        />
                        <div className="max-w-4xl mx-auto text-gray-800 dark:text-neutral-200">
                          <About data={entertainmentData} />
                          <EventTypes data={entertainmentData} />
                          <FAQ data={entertainmentData} />
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <SocialMediaLinks data={entertainmentData} />
    </>
  );
}
