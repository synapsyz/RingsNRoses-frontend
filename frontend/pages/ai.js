...
/**
 * Transforms the rich API response into a structured array for rendering.
 * @param {object} apiData - The raw data from the API.
 * @returns {array} An array of objects for the PlanRenderer component.
 */
const transformApiResponse = (apiData) => {
    ...
    // 2. Estimated Budget (if available)
    if (estimated_budget) {
        planItems.push({ type: 'budget', content: estimated_budget });
    }

    // 3. Detailed Service Recommendations
...
// NEW: Component to render the budget
...
// UPDATED: PlanRenderer now supports 'budget' and detailed 'recommendation' cards
                ...
                <div className="grid grid-cols-1 gap-3">
                  {item.content.vendors.map((vendor, i) => (
                    <a href={vendor.url} target="_blank" rel="noopener noreferrer" key={i} className="block bg-slate-100 dark:bg-neutral-800/60 border border-slate-200 dark:border-neutral-700 rounded-lg p-3 hover:border-blue-500 dark:hover:border-blue-500 transition-colors">
                      ...
                      <p className="text-xs text-slate-500 dark:text-neutral-400 mt-2 line-clamp-2">{vendor.about}</p>
        ...
        <div className="flex-grow overflow-y-auto pb-36 pt-10">
         <div className="max-w-3xl mx-auto px-4">
             <ul className="space-y-8">
                 ...
                 <li key={index} className="flex gap-x-4 animate-fade-in-up">
                     {message.sender === 'ai' ? <AIAvatar /> : null}

                     <div className={`w-full ${message.sender === 'user' ? 'flex justify-end' : ''}`}>
                     <div className={`max-w-lg ${message.sender === 'user' ? '' : 'p-4 bg-white dark:bg-neutral-800 rounded-2xl shadow-sm'}`}>
                         {message.type === 'thinking' && <ThinkingIndicator />}
                         {message.type === 'plan' && <PlanRenderer planItems={message.data.plan_items} />}
                         {message.type === 'text' && (
                     ...
                     {message.sender === 'user' ? <UserAvatar /> : null}
                 </li>
                 ))}
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-slate-100 dark:from-neutral-900 to-transparent p-4">
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            <textarea
              className="p-4 pr-20 block w-full border-slate-300 dark:border-neutral-700 rounded-xl text-md focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:text-neutral-200 dark:placeholder-neutral-400"
