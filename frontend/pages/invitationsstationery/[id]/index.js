...
export default function Home() {
  const [invitationsstationeryData, setInvitationsStationeryData] = useState(null);
  const [invitationsstationeryId, setInvitationsStationeryId] = useState(null);
  ...
  useEffect(() => {
    if (router.isReady && router.query.id) {
      ...
      setInvitationsStationeryId(router.query.id);
    }
  }, [router.isReady, router.query.id]);

  useEffect(() => {
    const fetchInvitationsStationeryData = async () => {
      setLoading(true);
      let loc_id = selectedLocationId || invitationsstationeryData?.location_details?.id;
      try {
        if (loc_id !== undefined && loc_id !== null) {
          ...
          const sug = await api.get(`/invitationsstationery/?location=${loc_id}`, config);
        ...
        console.error("Error fetching invitationsstationery data:", err);
        setError("Failed to load invitationsstationery data.");
      } finally {
        //setLoading(false);
      }
    };
    fetchInvitationsStationeryData();
  }, [accessToken, invitationsstationeryData, selectedLocationId]);

  useEffect(() => {
    if (!invitationsstationeryId) return;
    const fetchInvitationsStationeryData = async () => {
      setLoading(true);
      try {
        const config = {
        ...
        const response = await api.get(`/invitationsstationery/${invitationsstationeryId}/`, config);
        setInvitationsStationeryData(response.data);
        setIsFavorite(response.data.is_favorite || false);
        setShowContent(true);
        setError(null);
      } catch (err) {
        setError("Failed to load invitationsstationery data.");
        setInvitationsStationeryData(null);
    ...
    fetchInvitationsStationeryData();
  }, [invitationsstationeryId, accessToken]);

  useEffect(() => {
    if (invitationsstationeryData?.name) {
      setContent(
        `${invitationsstationeryData.name} is a delightful store offering a wide array of thoughtful presents for all your special occasions. The diverse selection of items available is mentioned below:`
      );
      setProductContent(
        `${invitationsstationeryData?.name} is perfect for your guests or event needs.`
      );
      setContactInfo({
        name: invitationsstationeryData?.manager_name || "N/A",
        number: invitationsstationeryData?.contact_number || "N/A",
        alternate_number: invitationsstationeryData?.alternative_number || "N/A",
      });
    }
  }, [invitationsstationeryData]);
  ...
  return (
    <>
      <Head>
        <title>
          {invitationsstationeryData?.name
            ? `${invitationsstationeryData.name} | invitationsstationery`
            : "Loading InvitationsStationery Details..."}
        </title>
        ...
        <main>
          ...
          <div id="content">
            <div className="w-full max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto">
              <div className="pt-6">
                <Breadcrumb data={invitationsstationeryData} />
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-y-10">
                  <div className="lg:col-span-3">
                    <ImageSlider data={invitationsstationeryData} />
                    ...
                    <div className="pt-14 pb-10">
                      <div className="mt-10">
                        <ServicesOffered
                          ...
                          content={content}
                          data={invitationsstationeryData?.services_offered_details}
                        />
              ...
              "lg": "#hs-sticky-sidebar-desktop-wrapper"
                    ...
                    >
                      <ProductDetails
                        ...
                        data={invitationsstationeryData}
      ...
      <SocialMediaLinks data={invitationsstationeryData} />
    </>
  );
}
