  ...
  const fetchInitialLocation = async (stateName, matchCity = '') => {
    try {
      ...
      const formattedLocations = data.results.map((loc) => ({
        value: loc.id,       // Store unique ID here
        label: loc.name,     // This is shown in the dropdown
        name: loc.name,      // Optional: you can keep the name separately too
      }));
...
const handleStateChange = async (selectedOption) => {
  ...
  if (selectedOption) {
    try {
      ...
      // 👇 Add this line to reset cache in case same search is repeated
      if (typeof loadLocationOptions === 'function') {
        loadLocationOptions(''); // optional: helps reset AsyncSelect on state change
      }
    } catch (err) {
              ...
              <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all space-y-4">
                ...
                {selectedState && (
                  <div>
                    ...
                    <AsyncSelect
                      key={selectedState?.value} // 👈 important to force re-render
                      cacheOptions
                      defaultOptions
