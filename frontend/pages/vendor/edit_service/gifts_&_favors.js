...
export default function AddGiftOrFavor() {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!giftOrFavorId) {
      setFormMessage({
        type: "error",
        text: "Cannot delete. Giftsfavors ID is missing.",
      });
      setIsActionCardVisible(true);
      return;
    }

    setFormMessage({ type: "info", text: "Deleting giftsfavors, please wait..." });
    ...
    try {
      ...
      await api.delete(`/giftsfavors/${giftOrFavorId}/`, config);
      setFormMessage({ type: "success", text: "Giftsfavors deleted successfully!" });
      0;
      setTimeout(() => {
        router.push("/vendor/service/preview");
      }, 2000);
    } catch (error) {
      console.error("Error trying to delete giftsfavors:", error);
      if (error.response) {
        setFormMessage({
          type: "error",
          text: `Error: ${
            error.response.data.detail || "Failed to delete giftsfavors."
          }`,
        });
      } else {
        setFormMessage({
