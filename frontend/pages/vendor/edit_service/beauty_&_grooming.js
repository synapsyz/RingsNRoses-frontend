...
export default function AddBeautyAndGroomingProduct() {
  ...
  const [cancellationPolicyContent, setCancellationPolicyContent] =
    useState("");
  ...
  const [beautyGroomingId, setBeautyGroomingId] = useState(null);
  ...
  const [beautyGroomingPackages, setBeautyGroomingPackages] = useState([]);
  const [minimumAdvanceBookingTime, setMinimumAdvanceBookingTime] =
    useState("");
  const [initialCertifications, setInitialCertifications] = useState([]);
  const [fetchedCertificateKey, setFetchedCertificateKey] = useState(null);
  const legalMediaManagerRef = useRef(null);
  const [updatedExistingCertifications, setUpdatedExistingCertifications] =
    useState([]);
  ...
  const handleEquipmentBlur = (id, value) => {
    const equipmentArray = String(value)
      .split(",")
      .map((item) => item.trim())
      .filter((item) => item !== "");
    ...
    setBeautyGroomingPackages((currentPackages) =>
      currentPackages.map((pkg) => {
        if (pkg.id === id) {
          const updatedEquipment = Array.from(
            new Set([...pkg.included_items, ...equipmentArray])
          );
      ...
      if (newTag) {
        setBeautyGroomingPackages((currentPackages) =>
          currentPackages.map((pkg) => {
            if (pkg.id === id) {
              const updatedEquipment = Array.from(
                new Set([...pkg.included_items, newTag])
              );
              console.log(
                `[KEY DOWN DEBUG] Package ID: ${id}, Equipment array TO BE SET (includes new tag):`,
                updatedEquipment
              );
              return {
                ...pkg,
                included_items: updatedEquipment,
                equipmentInput: "",
              };
            }
            return pkg;
    ...
    setBeautyGroomingPackages((currentPackages) =>
      currentPackages.map((pkg) => {
        if (pkg.id === packageId) {
          return {
            ...pkg,
            included_items: pkg.included_items.filter(
              (tag) => tag !== tagToRemove
            ),
          };
        }
  ...
  const deletePackage = (id) => {
    setBeautyGroomingPackages(
      beautyGroomingPackages.filter((pkg) => pkg.id !== id)
    );
  };
      ...
      if (beautyGroomingId) {
        try {
          ...
          const response = await api.get(
            `/beautygrooming/${beautyGroomingId}/`,
            config
          );
          ...
          if (editorInstance.current)
            editorInstance.current.commands.setContent(data.about || "");
          if (cancellationEditorInstance.current)
            cancellationEditorInstance.current.commands.setContent(
              data.cancellation_policy || ""
            );
          if (termsEditorInstance.current)
            termsEditorInstance.current.commands.setContent(
              data.terms_and_conditions || ""
            );
          if (returnDeliveryEditorInstance.current)
            returnDeliveryEditorInstance.current.setContent(
              data.return_delivery_policy || ""
            );
          ...
          if (data.services_offered_details) {
            setSelectedServices(
              new Set(
                data.services_offered_details.map((service) => service.id)
              )
            );
          }
          if (data.event_types_details) {
            setSelectedEventTypes(
              new Set(data.event_types_details.map((eventType) => eventType.id))
            );
          }
          ...
          if (data.packages && Array.isArray(data.packages)) {
            const loadedPackages = data.packages.map((pkg) => ({
              ...
              included_items: Array.isArray(pkg.included_items)
                ? pkg.included_items
                : [],
              isOpen: false,
              equipmentInput: "",
            }));
            setBeautyGroomingPackages(loadedPackages);
          } else {
            setBeautyGroomingPackages([]);
          }
        } catch (error) {
          ...
          setFormMessage({
            type: "error",
            text: "Failed to load beauty and grooming data.",
          });
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    beautyGroomingPackages.forEach((pkg) => {
      if (
        !pkg.pricing ||
        isNaN(parseFloat(pkg.pricing)) ||
        parseFloat(pkg.pricing) <= 0
      ) {
        newErrors[`packagePricing-${pkg.id}`] =
          "Pricing is required and must be a positive number.";
      }
    });
    ...
    let certificateKeyForPayload = fetchedCertificateKey;
    if (newCertificationFiles.length > 0) {
      ...
      if (!certificateUploadResult.success) {
        setFormMessage({
          type: "error",
          text: `Certificate upload failed: ${certificateUploadResult.message}`,
        });
        return;
      }
      ...
      updatedExistingCertifications.length === 0 &&
      fetchedCertificateKey
    ) {
      if (
    ...
    try {
      ...
      if (beautyGroomingId) {
        ...
        setFormMessage({
          type: "success",
          text: "Beauty & Grooming service updated successfully!",
        });
      } else {
        ...
        setFormMessage({
          type: "success",
          text: "Beauty & Grooming service added successfully!",
        });
      }
      ...
      if (error.response) {
        ...
        setFormMessage({
          ...
          text: `Error: ${
            ...
            "Failed to process Beauty & Grooming service."
          }`,
        });
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!beautyGroomingId) {
      setFormMessage({
        type: "error",
        text: "Cannot delete. beautyGrooming ID is missing.",
      });
      setIsActionCardVisible(true);
      return;
    }

    setFormMessage({ type: "info", text: "Deleting beautygrooming, please wait..." });
    ...
    try {
      ...
      await api.delete(`/beautygrooming/${beautyGroomingId}/`, config);
      setFormMessage({ type: "success", text: "beautygrooming deleted successfully!" });
      0;
      setTimeout(() => {
        router.push("/vendor/service/preview");
      }, 2000);
    } catch (error) {
      console.error("Error trying to delete beautygrooming:", error);
      if (error.response) {
        setFormMessage({
          type: "error",
          text: `Error: ${
            error.response.data.detail || "Failed to delete beautygrooming."
          }`,
        });
      } else {
        setFormMessage({
                ...
                <div className="lg:col-span-4 space-y-4">
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        ...
                        <FormInput
                          ...
                          label="Email Address"
                          type="email"
                          placeholder="glamourstudio@email.com"
                          value={emailAddress}
                      ...
                      <div>
                        ...
                        <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                          <TiptapEditor
                            ...
                            onUpdate={setAboutContent}
                            placeholder="Tell us about your beauty and grooming service..."
                          />{" "}
                          {/* Placeholder changed */}
                        </div>
                        {errors.aboutContent && (
                  ...
                  <MediaManager
                    ...
                    onUpdate={(existing, newFiles) => {
                      setUpdatedExistingMedia(existing);
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                      <h2 className="inline-block font-semibold text-stone-800 dark:text-neutral-200">
                        Beauty & Grooming Services Offered
                      </h2>{" "}
                      {/* Changed Title */}
                    </div>
                    <div className="p-4">
                  ...
                  <ServicePackages
                    packages={beautyGroomingPackages}
                    ...
                    sectionTitle="Beauty & Grooming Packages"
                    equipmentLabel="Products Used"
                    equipmentPlaceholder="e.g., Mac Foundation, Hair Spray, Facial Kit"
                    errors={errors}
                  />
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                    ...
                    >
                      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                        <TiptapEditor
                          ...
                          placeholder="Outline your terms and conditions for beauty and grooming services..."
                        />{" "}
                        {/* Placeholder changed */}
                      </div>
                    </div>
                    ...
                    >
                      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                        <TiptapEditor
                          ...
                          placeholder="Enter your beauty & grooming cancellation policy..."
                        />{" "}
                        {/* Placeholder changed */}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <div className="ml-4 mt-2 mr-4 mb-2 grid sm:grid-cols-3 gap-3 sm:gap-5">
                      ...
                      <FormInput
                        ...
                        type="url"
                        placeholder="https://instagram.com/yourbeautystudio"
                        value={instagramLink}
                      ...
                      <FormInput
                        ...
                        label="Facebook Link"
                        type="url"
                        placeholder="https://facebook.com/yourbeautystudio"
                        value={facebookLink}
                  ...
                  <div className="lg:sticky lg:top-5 space-y-4">
                    <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                      ...
                      <div className="p-5 space-y-4">
                        ...
                        />
                        <FormInput
                          id="minimumAdvanceBookingTime"
                          label="Minimum Advance Booking Time (Days)"
                          type="number"
                          placeholder="e.g., 5"
                          value={minimumAdvanceBookingTime}
                          onChange={(e) =>
                            setMinimumAdvanceBookingTime(e.target.value)
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                      <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                        <h2 className="inline-block font-semibold text-stone-800 dark:text-neutral-200">
                          Legal and Compliance (for verification, not public)
                        </h2>
                      </div>
                      <div className="p-5 space-y-4">
                        <CertificateManager
