...
export default function AddInvitationProduct() {
  ...
  const [invitationId, setInvitationId] = useState(null); // Changed from cateringId
  ...
  const [startingPrice, setStartingPrice] = useState(''); // Changed from perPlatePrice
...
console.log(serviceName);
 const handleGalleryUpdate = (existingMedia, newFiles) => {
  ...
  useEffect(() => {
    ...
    if (serviceId) {
      setInvitationId(serviceId); // Changed from setCateringId
    }
  }, [session]);

  useEffect(() => {
    const fetchInvitationData = async () => { // Changed from fetchCateringData
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await api.get("/services/venue/"); // Assuming same services endpoint or adjust if needed for invitations
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error adding/updating Invitation & Stationery:", error); // Changed message
      if (error.response) {
        console.error("Error data:", error.response.data);
        console.error("Error status:", error.response.status);
        console.error("Error headers:", error.response.headers);
        setFormMessage({ type: 'error', text: `Error: ${error.response.data.detail || 'Failed to process Invitation & Stationery.'}` }); // Changed message
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!invitationId) {
      setFormMessage({
        type: "error",
        text: "Cannot delete. Invitationsstationery ID is missing.",
      });
      setIsActionCardVisible(true);
      return;
    }

    setFormMessage({ type: "info", text: "Deleting invitationsstationery, please wait..." });
    ...
    try {
      ...
      await api.delete(`/invitationsstationery/${invitationId}/`, config);
      setFormMessage({ type: "success", text: "Invitationsstationery deleted successfully!" });
      0;
      setTimeout(() => {
        router.push("/vendor/service/preview");
      }, 2000);
    } catch (error) {
      console.error("Error trying to delete invitationsstationery:", error);
      if (error.response) {
        setFormMessage({
          type: "error",
          text: `Error: ${
            error.response.data.detail || "Failed to delete invitationsstationery."
          }`,
        });
      } else {
        setFormMessage({
                ...
                <div className="lg:col-span-4 space-y-4">
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                      <h2 className="inline-block font-semibold text-stone-800 dark:text-neutral-200">Service info</h2> {/* Changed label */}
                    </div>
                    <div className="p-5 space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        <FormInput id="InvitationName" label="Invitation Service Name" placeholder="ABC Invitations" value={Name} onChange={(e) => setName(e.target.value)} required error={errors.Name}/> {/* Changed label and placeholder */}
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        ...
                        <FormInput id="emailAddress" label="Email Address" type="email" placeholder="abcinvites@email.com" value={emailAddress} onChange={(e) => setEmailAddress(e.target.value)} required error={errors.emailAddress}/> {/* Changed placeholder */}
                      </div>
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        <FormInput
                      ...
                      <div>
                        ...
                        <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                          <TiptapEditor content={aboutContent} onUpdate={setAboutContent} placeholder="Tell us about your invitation & stationery service..." /> {/* Changed placeholder */}
                        </div>
                ...
                <div className="lg:col-span-2">
                  <div className="lg:sticky lg:top-5 space-y-4">
                    <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                      ...
                      <div className="p-5 space-y-4">
                        <FormInput id="startingPrice" label="Starting Price" type="number" placeholder="Enter Starting Price" value={startingPrice} onChange={(e) => setStartingPrice(e.target.value)} required error={errors.startingPrice}/> {/* Changed to startingPrice */}
                    ...
                    <AddressInput
                      heading="Address"
                      placeholder="Enter the full address of your invitation & stationery service." // Changed placeholder
                      value={address}
