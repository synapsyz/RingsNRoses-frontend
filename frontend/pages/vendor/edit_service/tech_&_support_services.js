...
export default function AddProduct() {
  ...
  const [techSupportServiceId, setTechSupportServiceId] = useState(null);
  const [isLocationModalOpen, setIsLocationModalOpen] = useState(false);
  const [selectedLocationData, setSelectedLocationData] = useState(null);
  const { data: session, status } = useSession();
  const [eventTypes, setEventTypes] = useState([]);
  const [selectedEventTypes, setSelectedEventTypes] = useState(new Set());
  const [services, setServices] = useState([]); // Services for Tech & Support
  ...
  useEffect(() => {
    ...
    if (serviceId) {
      setTechSupportServiceId(serviceId);
    }
  }, [session]);
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        // Assuming there's an API endpoint for tech and support services types
  ...
  useEffect(() => {
    const fetchTechSupportData = async () => {
      if (techSupportServiceId && services.length > 0 && eventTypes.length > 0) {
        try {
          const config = {
            headers: { Authorization: `Bearer ${session?.accessToken}` },
          };
          const response = await api.get(`/techsupportservices/${techSupportServiceId}/`, config); // **Update this API endpoint as needed**
          ...
          console.error("Error fetching tech support data:", error);
          setFormMessage({ type: 'error', text: 'Failed to load tech support data.' });
        }
      }
    };
    fetchTechSupportData();
  }, [techSupportServiceId, session, services, eventTypes]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (techSupportServiceId) {
        response = await api.put(`/techsupportservices/${techSupportServiceId}/`, formData, config); // **Update this API endpoint as needed**
        setFormMessage({ type: 'success', text: 'Tech & Support Service updated successfully!' });
      } else {
        response = await api.post("/techsupportservices/", formData, config); // **Update this API endpoint as needed**
        setFormMessage({ type: 'success', text: 'Tech & Support Service added successfully!' });
        if (response.data && response.data.id) {
          setTechSupportServiceId(response.data.id);
        }
      }
      console.log("Operation successful:", response.data);
    } catch (error) {
      console.error("Error adding/updating Tech & Support Service:", error);
      if (error.response) {
        console.error("Error data:", error.response.data);
        console.error("Error status:", error.response.status);
        console.error("Error headers:", error.response.headers);
        setFormMessage({ type: 'error', text: `Error: ${error.response.data.detail || JSON.stringify(error.response.data) || 'Failed to process Tech & Support Service.'}` });
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        <FormInput id="ServiceName" label="Name" placeholder="ABC Tech Solutions" value={Name} onChange={(e) => setName(e.target.value)} required />
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        ...
                        <FormInput id="emailAddress" label="Email Address" type="email" placeholder="abctech@email.com" value={emailAddress} onChange={(e) => setEmailAddress(e.target.value)} />
                      </div>
                      <div className="grid sm:grid-cols-2 gap-3 sm:gap-5">
                        <FormInput
                      ...
                      <div>
                        ...
                        <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                          <TiptapEditor content={aboutContent} onUpdate={setAboutContent} placeholder="Tell us about your tech & support services..." />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-2">
                  <div className="lg:sticky lg:top-5 space-y-4">
                    <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                      ...
                      <div className="p-5 space-y-4">
                        <FormInput id="startingPrice" label="Starting Price" type="number" placeholder="Enter starting price" value={startingPrice} onChange={(e) => setStartingPrice(e.target.value)} /> {/* ADD THIS LINE */}
