...
export default function AddTransportation() {
  ...
  // Removed eventSpaces and totalAreaSqft as they are not relevant for transportation
...
const handleEquipmentBlur = (id, value) => {
 ...
 const handleEquipmentKeyDown = (id, e) => {
  ...
  const deletePackage = (id) => {
    setTransportationPackages(transportationPackages.filter(pkg => pkg.id !== id));
  };
    ...
    const fetchTransportationData = async () => {
      if (transportationId) {
        try {
          ...
          const response = await api.get(`/transportation/${transportationId}/`, config); // Changed API endpoint
          ...
          // Removed eventSpaces and totalAreaSqft fetching
        ...
        //   setAdvancePaymentRequired(data.advance_payment_required || '');
          ...
          if (data.packages && Array.isArray(data.packages)) {
            ...
            setTransportationPackages(loadedPackages);
          } else {
            setTransportationPackages([]);
          }
        } catch (error) {
          console.error("Error fetching transportation data:", error);
          setFormMessage({ type: 'error', text: 'Failed to load transportation data.' });
        }
      }
    };
    fetchTransportationData();
  }, [transportationId, session, services, eventTypes]);

  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await api.get("/services/venue/"); // This should likely be /services/transportation/ or a more generic service endpoint
  ...
  const handleSubmit = async (e) => {
    ...
    if (transportationPackages.length > 0) { // Changed 'packages' to 'transportationPackages'
      const packagePrices = transportationPackages.map(pkg => parseFloat(pkg.pricing)).filter(price => !isNaN(price));
    ...
    const formData = {
      ...
      // Removed event_spaces and total_area_sqft from formData
    ...
    console.log("Submitting data for Transportation:", formData);
    ...
    try {
      ...
      let response;
      if (transportationId) {
        response = await api.put(`/transportation/${transportationId}/`, formData, config); // Changed API endpoint
      ...
      console.error("Error adding/updating Transportation service:", error);
      if (error.response) {
        console.error("Error data:", error.response.data);
        console.error("Error status:", error.response.status);
        console.error("Error headers:", error.response.headers);
        setFormMessage({ type: 'error', text: `Error: ${error.response.data.detail || 'Failed to process Transportation service.'}` });
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!transportationId) {
      setFormMessage({
        type: "error",
        text: "Cannot delete. Transportation ID is missing.",
      });
      setIsActionCardVisible(true);
      return;
    }

    setFormMessage({ type: "info", text: "Deleting transportation, please wait..." });
    ...
    try {
      ...
      await api.delete(`/transportation/${transportationId}/`, config);
      setFormMessage({ type: "success", text: "Transportation deleted successfully!" });
      0;
      setTimeout(() => {
        router.push("/vendor/service/preview");
      }, 2000);
    } catch (error) {
      console.error("Error trying to delete transportation:", error);
      if (error.response) {
        setFormMessage({
          type: "error",
          text: `Error: ${
            error.response.data.detail || "Failed to delete transportation."
          }`,
        });
      } else {
        setFormMessage({
                ...
                <div className="lg:col-span-4 space-y-4">
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                      <h2 className="inline-block font-semibold text-stone-800 dark:text-neutral-200">Transportation Service Info</h2>
                    </div>
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    <div className="py-3 px-5 flex justify-between items-center gap-x-5 border-b border-stone-200 dark:border-neutral-700">
                      <h2 className="inline-block font-semibold text-stone-800 dark:text-neutral-200">Transportation Services Offered</h2>
                    </div>
                    <div className="p-4">
                      <CheckboxGroup
                        items={services}
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <div id="hs-add-product-Event-supported-card-body" className="p-5 space-y-4">
                      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                        <TiptapEditor content={termsAndConditions} onUpdate={setTermsAndConditions} placeholder="Outline your terms and conditions for transportation services..." />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <div id="hs-add-product-Event-supported-card-body" className="p-5 space-y-4">
                      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                        <TiptapEditor content={cancellationPolicy} onUpdate={setCancellationPolicy} placeholder="Enter your transportation cancellation policy..." />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col bg-white border border-stone-200 overflow-hidden rounded-xl shadow-2xs dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <div className="ml-4 mt-2 mr-4 mb-2 grid sm:grid-cols-3 gap-3 sm:gap-5">
                      ...
                      <FormInput id="instagramLink" label="Instagram Link" type="url" placeholder="https://instagram.com/yourtransportation" value={instagramLink} onChange={(e) => setInstagramLink(e.target.value)} />
                      <FormInput id="facebookLink" label="Facebook Link" type="url" placeholder="https://facebook.com/yourtransportation" value={facebookLink} onChange={(e) => setFacebookLink(e.target.value)} />
                    </div>
                  </div>
                </div>
