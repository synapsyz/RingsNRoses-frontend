...
const handleGalleryUpdate = (existingMedia, newFiles) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!entertainmentId) {
      setFormMessage({
        type: "error",
        text: "Cannot delete. Entertainment ID is missing.",
      });
      setIsActionCardVisible(true);
      return;
    }

    setFormMessage({ type: "info", text: "Deleting entertainment, please wait..." });
    ...
    try {
      ...
      await api.delete(`/entertainment/${entertainmentId}/`, config);
      setFormMessage({ type: "success", text: "Entertainment deleted successfully!" });
      0;
      setTimeout(() => {
        router.push("/vendor/service/preview");
      }, 2000);
    } catch (error) {
      console.error("Error trying to delete entertainment:", error);
      if (error.response) {
        setFormMessage({
          type: "error",
          text: `Error: ${
            error.response.data.detail || "Failed to delete entertainment."
          }`,
        });
      } else {
        setFormMessage({
